...
class SaintyCheckLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = torch.view_as_real(
            torch.stft(target, self.window_size, self.stride, window=self.hann, return_complex=True)).permute(2, 0, 1)
        sig = torch.view_as_real(
            torch.stft(sig, self.window_size, self.stride, window=self.hann, return_complex=True)).permute(2, 0, 1)
...
class BlindTestLoader(Dataset):
    def __init__(self, test_dir, compute_metrics: bool = False):
        ...
        self.sr = CONFIG.DATA.sr
        self.stride = CONFIG.DATA.stride
        self.window_len = CONFIG.DATA.window_size
        self.hann = torch.sqrt(torch.hann_window(self.window_len))
    ...
    def __getitem__(self, index):
        ...
        sig_wav = sig.clone()
        sig = torch.view_as_real(
            torch.stft(sig, self.window_len, self.stride, window=self.hann, return_complex=True)).permute(2, 0, 1)
        if self.compute_metrics:
            ...
            target = torch.view_as_real(
                torch.stft(target, self.window_len, self.stride, window=self.hann, return_complex=True)).permute(2, 0,
                                                                                                                 1)
...
class NonBlindTestLoader(Dataset):
    ...
    def fetch_audio(self, index):
        target = load_audio(self.data_list[index], sample_rate=self.sr)
        assert target.shape[0] > self.p_size, f"Expected to get audio with the size more than: {self.p_size}, " \
                                              f"got: {target.shape[0]}."
        target = target[:(target.shape[0] // self.p_size) * self.p_size]
        return target

    def __getitem__(self, index):
        sig = self.fetch_audio(index)
        if self.mask == 'real':
            # mask = np.array(self.trace_list[index])
            name = Path(self.data_list[index]).stem
            txt_path = Path(CONFIG.NBTEST.loss_path) / f"{name}_is_lost.txt"
            mask = np.array([1 - np.array(list(map(int, open(txt_path, 'r').read().strip('\n').split('\n'))))])
            mask = torch.repeat_interleave(torch.tensor(mask, dtype=torch.float32), self.p_size)
            # assert mask.shape[0] >= sig.shape[0], f"Expected to mask after upsampling to be same or longer " \
            #                                       f"than sig, got: {mask.shape} and {sig.shape}, " \
            #                                       f"{self.data_list[index]}"
            mask = mask[:sig.shape[0]]
            assert mask.shape == sig.shape, f"Expected to be the same len, got: {mask.shape}, {sig.shape}, " \
                                            f"{self.data_list[index]}"
        else:
            print("Generating mask...")
            mask_initial_dim = sig.shape[0] // self.window_size
            mask = self.mask_generator.gen_mask(mask_initial_dim, seed=index)
            mask = torch.repeat_interleave(torch.tensor(mask.copy(), dtype=torch.float32), self.window_size)
        sliced_sig = slice_into_frames(sig, self.window_size, self.stride)  # p_size, n_frames
        masked_sig = sig * mask.cpu().numpy()
        masked_sliced_sig = slice_into_frames(masked_sig, self.window_size, self.stride)  # p_size, n_frames
        sliced_mask = slice_into_frames(mask.cpu().numpy(), self.window_size, self.stride)  # p_size, n_frames
        if sliced_sig.shape != sliced_mask.shape:
            print(f"Caution {self.data_list[index]}")
        # take min values intersections (if hop from present and absent signal, take as absent)
        mask1d = torch.min(torch.tensor(sliced_mask), dim=0).values
        # print(mask1d)

        # stft
        weights_window = librosa.filters.get_window('hann', self.window_size, fftbins=True)
        masked_stft = np.fft.rfft(np.einsum('ij, i -> ij', masked_sliced_sig, weights_window),
                                  n=weights_window.shape[0],
                                  axis=0)
        masked_stft = torch.view_as_real(torch.tensor(masked_stft)).permute(2, 0, 1).float()
        target_stft = np.fft.rfft(np.einsum('ij, i -> ij', sliced_sig, weights_window),
                                  ...
                                  axis=0)
        target_stft = torch.view_as_real(torch.tensor(target_stft)).permute(2, 0, 1).float()
        return masked_stft, target_stft, mask1d, Path(self.data_list[index]).name
